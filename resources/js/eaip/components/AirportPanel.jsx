...
import airportPanelBg from '../assets/airport-images/airportpanel.jpg';

function AirportPanel({ airport: initialAirport, onClose, darkMode }) {
  ...
  // Fetch detailed airport data (including charts) and METAR
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Parallel fetch for details and METAR
        const [detailsRes, metarRes] = await Promise.allSettled([
          axios.get(`${API_BASE}/api/airport/${airport.icao}`),
          axios.get(`${API_BASE}/api/metar/${airport.icao}`)
        ]);

        if (detailsRes.status === 'fulfilled' && detailsRes.value.data.success) {
          setAirport(prev => ({ ...prev, ...detailsRes.value.data.data }));
        }

        if (metarRes.status === 'fulfilled' && metarRes.value.data.success) {
          setMetar(metarRes.value.data.data);
        }
      } catch (err) {
    ...
    fetchData();
  }, [initialAirport.icao]); // Rely on initial ICAO to trigger refetch if prop changes
  ...
  return (
    <div className={`airport-panel ${darkMode ? 'dark' : 'light'}`}>
      ...
      <div
        ...
        style={{
          backgroundImage: `url(${airportPanelBg})`,
...
function ChartsContent({ airport }) {
  ...
  // Use charts from prop, default to empty array
